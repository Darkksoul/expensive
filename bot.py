...
async def start_handler(c: Client, m: Message):
    ...
    if m.from_user.id != int(Config.OWNER):
        ...
        user.set()
    res = await m.reply_text(
        text=f"Hi **{m.from_user.first_name}**\n\n ⚡ I am a file/video merger bot\n\n😎 I can merge Telegram files!, And upload it to telegram @DevilServers\n\n**Owner: 🈲 @{Config.OWNER_USERNAME}** ",
        quote=True,
    )
    del user
...
async def makeButtons(bot: Client, m: Message, db: dict):
    ...
    markup.append([InlineKeyboardButton("🔗 Merge Now @DevilServers", callback_data="merge")])
    markup.append([InlineKeyboardButton("💥 Clear Files @DevilServers", callback_data="cancel")])
    return markup
    ...
    try:
        with userBot:
            userBot.send_message(
                ...
                text="Bot booted with Premium Account,\n\n  Thanks for using <a href='@DevilServers'> Devil </a>",
